...
Captured against `origin/main` @ `7db242c0d7f8` (2026-09-18). This doc is not
...
| opengameinstaller | `packages/opengameinstaller.nix` | OpenGameInstaller front-end: upstream AppImage unpacked with `appimageTools.extract`, its `resources/app.asar` run on nixpkgs `electron_42` (the bundled 40.10.2 is EOL in nixpkgs; the asar's native modules are all N-API, so the major is free) with no bubblewrap sandbox, so the Play button's launch chain runs in Steam's environment; wrapper execs `systemd-cat -t opengameinstaller --stderr-priority=warning` in front of `electron`, so OGI's stdout/stderr (including `[umu]` lines) land in the journal instead of vanishing when the process is launched from a `.desktop` entry — read them with `journalctl -t opengameinstaller`, this package's replacement for the `update/latest.log` the upstream `-Setup.AppImage` would have produced and which this package deliberately does not ship; wrapper also sets `ELECTRON_FORCE_IS_PACKAGED=1` (keeps `app.isPackaged` true on a bare `electron` binary) and `APPIMAGE=/run/current-system/sw/bin/opengameinstaller-steam-launch`, a second binary from the same derivation (`packages/opengameinstaller-steam-launch.sh`) that OGI's Steam/desktop shortcut writers point their shortcuts at; it reconstructs and execs a managed game's launch chain directly from `library/<id>.json` (per-game env, UMU wine prefix, DLL overrides, launch arguments) and logs to `journalctl -t opengameinstaller-steam-launch`, so a Steam-managed launch no longer starts Electron at all; callpackaged in `modules/nixos/console/launchers.nix` |
