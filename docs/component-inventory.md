...
| opengameinstaller | `packages/opengameinstaller.nix` | OpenGameInstaller front-end, built from the `shockstruck/OpenGameInstaller` fork's `v4.3.1-ss.5` release (fork of `Nat3z/OpenGameInstaller`, same `Build/release` AppImage): AppImage unpacked with `appimageTools.extract`, its `resources/app.asar` run on nixpkgs `electron_42` (the bundled 40.10.2 is EOL in nixpkgs; the asar's native modules are all N-API, so the major is free) with no bubblewrap sandbox — needed for OGI's own Play button (pressure-vessel via the umu zipapp), since Steam-managed shortcuts no longer route through OGI at launch as of this release (OGI writes the launch environment into the shortcut's `LaunchOptions` and Steam starts the game directly); wrapper execs `systemd-cat -t opengameinstaller --stderr-priority=warning` in front of `electron`, so OGI's stdout/stderr (including `[umu]` lines) land in the journal instead of vanishing when the process is launched from a `.desktop` entry or Steam shortcut — read them with `journalctl -t opengameinstaller`, this package's replacement for the `update/latest.log` the upstream `-Setup.AppImage` would have produced and which this package deliberately does not ship; wrapper also sets `ELECTRON_FORCE_IS_PACKAGED=1` (keeps `app.isPackaged` true on a bare `electron` binary) and `APPIMAGE=/run/current-system/sw/bin/opengameinstaller` so OGI's per-game `.desktop` entries (`Exec=<launcher> --game-id=N`) and the Steam re-sync's legacy-executable match still resolve the launcher (Steam shortcuts themselves no longer exec it on Linux); callpackaged in `modules/nixos/console/launchers.nix` |
