...
Captured against `origin/main` @ `8df8794` (2026-09-14). This doc is not
...
| `console/default.nix` | Imports `./session.nix`, `./performance.nix`, `./input.nix`, `./launchers.nix`, `./streaming.nix`, `./desktop.nix`, `./decky.nix`; boot quiet/plymouth settings (no `services.xserver.enable` — gamescope needs no X server stack). Console-only: reaches neither desktop nor laptop |
...
| `console/decky.nix` | `systemd.services.decky-loader` (root; setuid's to the unprivileged user per plugin per decky-loader issue 446; `UNPRIVILEGED_USER`/`UNPRIVILEGED_PATH`/`PLUGIN_PATH` point at the console's Steam user and `/var/lib/decky-loader`, matching Unifideck's own `~/.local/share/unifideck/…` reads), `systemd.tmpfiles.settings` linking the callpackaged `unifideck` derivation into `${stateDir}/plugins/Unifideck` as a store symlink (declarative; decky-loader's own updater can't write through it, so plugin updates are a version bump in `packages/unifideck.nix`), `programs.nix-ld.enable` (Unifideck vendors two prebuilt glibc ELF binaries, `nile`/`comet`, under its `bin/`) |
...
| decky-loader | `packages/decky-loader.nix` | Decky Loader 3.2.8, vendored from Jovian-NixOS's `pkgs/decky-loader/default.nix` (pnpm/Python `buildPythonPackage`); `postPatch` adds two `--replace-fail` edits to `backend/decky_loader/helpers.py` (absolute store `python3`, `$PATH` passthrough) so `get_system_pythonpaths()` works outside a login shell; `systemd` added to the wrapper's PATH so decky's bare `systemctl` calls resolve; callpackaged in `modules/nixos/console/decky.nix` |
| unifideck | `packages/unifideck.nix` | Unifideck 0.7.5 Decky plugin, installed from upstream's release zip (already carries `dist/`, vendored `py_modules/`, and the store CLIs `legendary`/`gogdl`/`nile`/`comet`/`unifideck-launcher`) rather than built from source; `$out` is the plugin root (`$out/plugin.json`); `nile`/`comet` are unpatched glibc ELF binaries relying on `programs.nix-ld` at runtime; linked into decky-loader's `plugins/` as a store symlink by `modules/nixos/console/decky.nix` |
